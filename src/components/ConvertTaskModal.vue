...
const filteredTasks = computed(() => {
    ...
    return todoStore.allItems
        .filter(item =>
            item.id !== props.todoId &&
            item.title.toLowerCase().includes(query)
        )
        .sort((a, b) => b.updatedAt - a.updatedAt)
})

const handleConvert = (targetTodoId: string) => {
    emit('convert', targetTodoId)
}
</script>
    ...
    <div v-if="isOpen" class="move-modal-overlay" @click="emit('close')">
        <div class="move-modal-content" @click.stop>
            <div class="move-header">
                <h3 class="move-title">{{ t('modal.convertTask') }}</h3>
            ...
            <div class="subtask-preview">
                <p class="preview-label">{{ t('modal.convertingTask') }}:</p>
                <p class="preview-title">{{ todoTitle }}</p>
            </div>
            ...
            <div class="tasks-list">
                ...
                <button v-for="task in filteredTasks" :key="task.id" class="task-item" @click="handleConvert(task.id)">
                    <div class="task-info">
                        <div class="task-line">
                            <span class="task-name">{{ task.title }}</span>
                            <span v-if="task.status === 'completed'" class="completed-badge">{{
                                t('tasks.status.completed') }}</span>
                        </div>
